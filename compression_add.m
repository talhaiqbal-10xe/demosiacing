...
while(start>1)
    out=strcat(out,'\n\n\n\n','///////////// Iteration Number =',num2str(iteration),'///////////////////','\n\n\n\n');
    for i=length(inp):-1:2
        bits=inp(i).bits;
        name=convertStringsToChars(inp(i).names);
        if (bits>start)
            if (bits-1==start)
                [bits,name,name_next,out_temp]=half_adder(bits,name,iteration,-1*i+length(inp));
                inp(i).bits=bits;
                ...
                while(bits~=start)
                    if (bits-1==start)
                         ...
                         [bits,name,name_next,out_temp]=full_adder(bits,name,iteration,-1*i+length(inp),number);
                         number=number+1;
                         inp(i).bits=bits;
                         inp(i).names=convertStringsToChars(name);
                         out=strcat(out,'\n\n',out_temp);
